...
the best, sometimes they are good, but old. If you want to have access to the
...
Once done with the update, you can install the latest [NVIDIA][nvidia] driver
like so:
...
To check if you have [Vulkan][vulkan] working correctly you will need
`vulkan-utils` package:
...
NOTE: If [Vulkan][vulkan] is not working after the install, you might have to do
a simple system reboot, which you can do via the terminal with `sudo reboot`

## Optional

If you have [Dota 2][dota2] on your machine, you can use it to check for
[Vulkan][vulkan].
